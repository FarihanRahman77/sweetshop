...
class SaleReturnController extends Controller
{
    public function saleReturn($id)
	{
		...
		// $saleProducts = SaleProduct::where('sale_id', $sale->id)
		// ->where('deleted', 'No')
		// ->where('status', 'Active')
		// ->get();
		...
		$saleProducts = DB::table('tbl_restaurant_order_details')
			...
			->select('tbl_restaurant_order_details.*', 'tbl_inventory_products.name as productname', 'tbl_inventory_products.sale_price', 'tbl_inventory_products.id as product_id', 'tbl_settings_company_settings.name')
	...
	public function deleteSaleReturn(Request $request)
	{
		...
		try {
			...
			PaymentVoucher::where('sales_return_id', $request->id)->update(['deleted' => 'Yes', 'deletedBy' => auth()->user()->id, 'deletedDate' => date('Y-m-d H:i:s')]);
