...
@section('title')
    {{ Session::get('companySettings')[0]['name'] }} Add Asset Sales
@endsection
@section('content')
    ...
    <section class="content box-border">
        <div class="card">
            <div class="card-header">
                <h3>Asset Sale 
                    <a class="btn btn-primary float-right" href="{{ route('assets.sales.view') }}">
                        Back To Asset Sales List <i class="fa fa-reply"></i>
                    </a>
                </h3>
            </div><!-- /.card-header -->
            <div class="card-body">
                <form>
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-md-2">
                            ...
                            <input type="date" id="sell_date" name="sell_date" class="form-control input-sm" value="{{ date('Y-m-d') }}" />
                        </div>
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <input id="customer_name" name="customer_name" class="form-control input-sm" placeholder="Customer Name">
                            <span class="text-danger" id="customer_nameError"></span>
                        </div>
                        <div class="form-group col-md-3">
                            <label>Chart Of Accounts: <span class="text-danger">*</span></label>
                            <select id="coa_id" name="coa_id" class="abcd" style="width:100%">
                                <option value='' selected='true'> Select Chart Of Accounts </option>
                                @foreach ($coas as $coa)
                      ...
                      <!--   <div class="form-group col-md-2">
                        ...
                        <div class="input-group col-md-12 row">
                            <div class="col-md-6">
                                ...
                                <select id="products" name="products" class="form-control input-sm" style="width:96%" onchange="getSerializeProduct()">
                                    <option value=""> Product Search </option>
                                    @foreach ($products as $product)
                                        <option value="{{ $product->id }}">
                                            {{ $product->productName . ' - ' . $product->productCode }} </option>
                                    @endforeach
                                </select>
                                <span class="text-danger" id="productIdError"></span>
                            </div>
                            <div class="col-md-6" id="serialProductsDiv">
                            ...
                            <table border="1" style="font-size: 13px; width:100%;" class="table-bordered">
                                ...
                                <tr>
                                    <td colspan="5" class="text-right">Paid Amount
                                        {{ Session::get('companySettings')[0]['currency'] }} : </td>
                                    <td><input type="text" class="text-right only-number form-control" value="0"
                                            id="payment" name="payment" readonly/></td>
                                    <td></td>
                                </tr>
                            </table>
                        </div>
    ...
    <div class="modal-dialog modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add Amotization</h4>
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i
                        class="fas fa-window-close"></i></button>
            </div>
...
<script>
   ...
   jQuery(document).on('change','.depreciation',function () {
   
       var  depreciation =   $(".depreciation").val();
  
       if(depreciation == "One Time Pay") {
        $('.no_of_month').prop('disabled', true);
        $('.no_of_month').val(0);
       }else{
        $('.no_of_month').prop('disabled', false);
       }
    ...
    function getSerializeProduct(){
        var product_id=$('#products').val();
        //alert(products);
        $.ajax({
                url: "{{ route('assets.sales.getSerializeProduct') }}",
                method: "GET",
                data: {
                ...
                success: function(result) {
                    ...
                    $('#serialProductsDiv').html(result);
                    $("#serialize_product_id").select2({
                        placeholder: " Choose Serialize Products ",
                        /*dropdownParent: $("#modal"),*/
                        allowClear: true,
                        width: '100%'
                    });
    ...
    function fetchCart() {
        //alert('bdcvcfvbf');
        $.ajax({
            url: "{{ route('assets.sales.fetchCart') }}",
            method: "get",
            success: function(result) {
                //alert(JSON.stringify(result.data.totalAmount));
               // alert(JSON.stringify(result));
                $("#manageCartTable").html(result.data.cart);
                
                $("#grandTotal").text(result.data.totalAmount);
                calculateTotal();
            },
            beforeSend: function() {
                $('#loading').show();
            ...
            error: function(response) {
                //alert(JSON.stringify(response));
                /* $("#products_error").text("No such product available in your system 1 " + JSON
                    .stringify(
                        response)); */
            }
        })
    ...
    function showSerializTable(id, warehouseId) {
        ...
        fd.append('_token', _token);
        $.ajax({
            url: "{{ route('assets.purchase.showSerializTable') }}",
            method: "POST",
            data: fd,
            contentType: false,
            processData: false,
    ...
    function addRow() {
        ...
        rows +=
            ...
            ' readonly  ><td><a href="#" onclick="removeRow(' +
    ...
    function add_to_serialize_cart() {
        var id = $("#serialize_product_id").val();
        if (!id) {
            return 0;
        }
    ...
    $("#supplier").change(function() {
        ...
        fd.append('_token', _token);
        $.ajax({
            url: "{{ route('assets.purchase.supplierDue') }}",
            method: "GET",
            data: fd,
            contentType: false,
            processData: false,
    ...
    function addToCart(fd) {
        $.ajax({
            url: "{{ route('assets.sales.addToCart') }}",
            method: "POST",
            data: fd,
            contentType: false,
            processData: false,
    ...
    function updateCart(id, product_type = null) {
        ...
        fd.append('id', id);
        $.ajax({
            url: "{{ route('assets.sales.updateCart') }}",
            method: "POST",
            data: fd,
            contentType: false,
            processData: false,
    ...
    function loadCartandUpdate(id, product_type = null) {
        
        updateCart(id, product_type);
    }
    ...
    function clearCart() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                fd.append('_token', _token);
                $.ajax({
                    url: "{{ route('assets.sales.clearCart') }}",
                    method: "POST",
                    data: fd,
                    contentType: false,
                    processData: false,
                ...
                Swal.fire("Cancelled", "Your imaginary Cart is safe :)", "error");
            }
        })
    }
    ...
    function removeCartProduct(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                fd.append('_token', _token);
                $.ajax({
                    url: "{{ route('assets.sales.removeProduct') }}",
                    method: "POST",
                    data: fd,
                    contentType: false,
                    processData: false,
    ...
    function checkOutCart() {
        ...
        //Just For Validation
        fd.append('product_id', product_ids); //-product_id as product_name
        fd.append('customer_name', customer_name); //-supplier_id as supplier_name
        ...
        fd.append('coa_id', coa_id);
        fd.append('payment_method', payment_method);
        fd.append('_token', _token);
        clearErrorMessage();
        if(customer_name == ''){
            $('#customer_nameError').text('Customer name can not empty.');
        }else{
            $.ajax({
            url: "{{ route('assets.sales.checkOutCart') }}",
            method: "POST",
            data: fd,
            contentType: false,
            processData: false,
            ...
            success: function(result) {
               // alert(JSON.stringify(result));
                let purchaseId = result.assetSaleId;
                clearSalesForm();
                fetchCart();
            ...
            error: function(response) {
                ...
                $('#customer_nameError').text(response.responseJSON.errors.customer_name);
