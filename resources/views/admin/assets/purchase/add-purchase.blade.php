...
@section('title')
{{ Session::get('companySettings')[0]['name'] }} Add Asset Products
@endsection
@section('content')
    ...
    <section class="content box-border">
        <div class="card">
            <div class="card-header">
                <h3>Asset Purchase
                    <a class="btn btn-primary float-right" href="{{ route('assets.purchase.index') }}">
                ...
                <form>
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <select id="sister_concern" name="sister_concern" class="form-control" style="width:100%" required>
                            ...
                            <span class="text-danger" id="sister_concernError"></span>
                        </div>
                        <div class="form-group col-md-4">
                        ...
                        <div class="input-group col-md-9">
                            ...
                            <select id="products" name="products" class="form-control input-sm" style="width:96%">
                                ...
                                <option value="{{ $product->id }}">
                                    {{ $product->productName . ' - ' . $product->modelNo }} </option>
                                @endforeach
                            </select>
                            ...
                            <table border="1" style="font-size: 13px; width:100%;" class="table-bordered">
                                ...
                                <tr>
                                    <td colspan="7" class="text-right">Paid Amount
                                        {{ Session::get('companySettings')[0]['currency'] }} : </td>
                                    <td><input type="text" class="form-control text-right only-number" value="0" id="payment"
                                            name="payment" /></td>
                                    <td></td>
                                </tr>
                            </table>
                    ...
                    <div class="row">
                        <div class="form-group ">
                            ...
                            <table border="1">
                                ...
                                <input type="hidden" id="serializProductWarehouseId" name="serializProductWarehouseId"
                                    value="">
...
function fetchCart() {
    $.ajax({
        url: "{{ route('assets.purchase.fetchCart') }}",
        method: "get",
        success: function(result) {
...
function addToCart(fd) {
    $.ajax({
        url: "{{ route('assets.purchase.addToCart') }}",
        method: "POST",
        data: fd,
        contentType: false,
        processData: false,
...
function updateCart(id, warehouse_id, product_type = null) {
    ...
    var _token = $('input[name="_token"]').val();
    var depreciation = $('#depreciation_' + id + '_' + warehouse_id).val();
    var no_of_month = $('#no_of_month_' + id + '_' + warehouse_id).val();
    ...
    fd.append('depreciation', depreciation);
    fd.append('no_of_month', no_of_month);
    fd.append('unitPrice', unitPrice);
    fd.append('warehouseId', warehouse_id);
    fd.append('_token', _token);
    fd.append('id', id);
    $.ajax({
        url: "{{ route('assets.purchase.updateCart') }}",
        method: "POST",
        data: fd,
        contentType: false,
        processData: false,
...
function clearCart() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            fd.append('_token', _token);
            $.ajax({
                url: "{{ route('assets.purchase.clearCart') }}",
                method: "POST",
                data: fd,
                contentType: false,
                processData: false,
...
function removeCartProduct(id, warehouse_id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            fd.append('_token', _token);
            $.ajax({
                url: "{{ route('assets.purchase.removeProduct') }}",
                method: "POST",
                data: fd,
                contentType: false,
                processData: false,
...
function checkOutCart() {
    ...
    var sisterconcern = $("#sister_concern").val();
    ...
    fd.append('selected_sister_concern', sisterconcern);
    ...
    fd.append('n_month', n_month);
    fd.append('depre', depre);
    fd.append('_token', _token);
    clearErrorMessage();
    $.ajax({
        url: "{{ route('assets.purchase.checkOutCart') }}",
        method: "POST",
        data: fd,
        contentType: false,
        processData: false,
